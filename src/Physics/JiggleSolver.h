...
// A single Euler step's behavior depends on the frame's dt (explicit Euler's error and effective damping scale with dt), 
...
namespace OSF::Physics
{
	...
	struct BoneState
	{
		...
		float        dtAccum{ 0.0f };  // unspent time carried toward the next fixed sub-step
